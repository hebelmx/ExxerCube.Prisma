...
| **Docker** (Desktop/Engine) | Testcontainers SQL suites + the gate (default path) | Pulls `mcr.microsoft.com/mssql/server:2025-latest` (2022 retired) and (for some suites) Ollama. **Optional** for the gate if you use a local SQL instance instead — see §6.2. |
| **SQL Server 2022+** *(alternative to Docker)* | the gate's `PRISMA_GATE_LOCAL_SQL` path | Ledger tables need SQL Server **2019 CU13+/2022+** (Testcontainers fixture uses **2025**). A local `DESKTOP\SQL2025` instance with Integrated Security works. |
...
| **Native Tesseract + tessdata** | real-OCR tests + the gate | Language data **`spa`** (primary) **+ `eng`** must be resolvable by the Athena worker (its working dir `./tessdata` or `TESSDATA_PREFIX`). **Linux:** `apt install tesseract-ocr tesseract-ocr-spa tesseract-ocr-eng`, then `export TESSDATA_PREFIX=/usr/share/tesseract-ocr/5/tessdata/`. The charlesw wrapper wants Ubuntu-22.04 sonames; a Linux-only post-build target (`eng/link-tesseract-natives.sh`) auto-symlinks the system libs — **no manual step**. |
| **Emgu.CV / OpenCV native** *(Linux only)* | imaging-quality + real-OCR-fusion suites (`Tests.Infrastructure.Imaging`); **not** the Worker/gate, which use the non-Emgu `PolynomialImageQualityAnalyzer` | Uses the **distro-pinned** `Emgu.CV.runtime.ubuntu-26.04-x64` (Emgu.CV 4.13). The native `libcvextern.so` needs system libs once: `apt install libvtk9.5 libhdf5-310 libavif16 libgeotiff5 liblapack3`. `UseRidGraph=true` (Linux) + a base-dir symlink target make the native loadable — **no manual step** beyond the apt libs. **If the box's Ubuntu version differs from 26.04, swap the runtime package to the matching `ubuntu-XX.XX-x64`** and re-check `ldd libcvextern.so`. |
