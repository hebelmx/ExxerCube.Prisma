...
**Date opened:** 2026-07-22 · **Branch:** `Liv` · **Status:** **DONE (closed 2026-07-22, second orchestrated session)** — all 5 stories complete, S4.b adversarial gate passed, S5 pins green.
...
| RC1.S4 | **Evidence-gated fixes.** Owner authorized all 3 tracks 2026-07-22 + ruled: CL-46 → SAT "…DE UN CFDI" legend; PLATINUM catalog row in BOTH catalogs; **§26-i → tolerate as accepted variant**; **TYPO footnote markers → stay IN scope (fail stands — first owner-ratified true finding on real statements)**. Sub-stories: **S4.a rules DONE** (`c377788b`: CL-18 real fails 4→1, §26 4→0, CL-21 grounded-zero abstains; demo pin CL-21→CL-22, RED preserved — forced by evidence, labels absent from that PDF family) · **S4.b DONE in 4 chunks B1/B4/B2/B3 (`4d550573`/`71e8b8fc`/`4c55c93f`/`a44eff25`) + adversarial gate PASSED ("COMPLETE WITH GAPS — safe to pin"; every floor + delta independently reproduced, zero PII in range, no fabricated Pass). Gate-mandated documented residuals (accepted, corpus-bounded allowlists fail LOUD not safe): (i) CL-18 fee-prefix allowlist covers only this corpus's 5 fee phrasings — an uncovered real bank-fee phrasing (cash-advance/replacement-card/FX commission) will Critical-Fail, mirrored original bug; (ii) CL-50/51/52 fiscal-legend fast-path is a 2-variant allowlist, same risk shape; (iii) CL-42 blind spot: ChargeDate==null + OperationDate<start (prior-period mis-billing) passes silently.** · **S4.c data DONE** (`500e83a6`: SAT CFDI legend both catalogs — CL-46 B×4→Pass, C-2026-02 honest residual; TC-PLATINUM row both catalogs — C-series ExtractionGap→Red×4, real rules running; owner-TODO: PLATINUM annualCommission/rewards ungrounded) · **S4.d corpus DONE** (`4e28136a`+`1c81836e`, 3 rounds: auto-shrink, global-map leak fix (S1 gate caught it), boundary-aware placement + PyMuPDF spurious-hit guard; CL-28 artifact eliminated, LAW-TYPO ×4 retained as owner-ratified genuine finding). **Residual:** CL-18 still fails 1/4 (B-2026-06) — triage before S5 pins. **S4.b progress (2026-07-22b):** chunk B1 DONE (`4d550573`: CL-31 page-label plausibility gate → real Fail×8→InsufficientData×8; CL-48 image-aware gap — image-blindness fixed but B-series STILL Fail×8: one ~211pt truly-empty region/doc (no text, no image) → triage's blanket "(c)" was partially wrong, CL-48 residual carried to S5 pending visual/owner review; possible vector graphics invisible to PdfPig). **Residual triage DONE (read-only pass):** CL-18 B-2026-06 = (c) `\bCOMISION\b` in `MovementClassifier` matched a MERCHANT name (Comisión Estatal de Aguas, −$230.00 exact; CFE-scale landmine) → fix = bank-fee prefix anchoring; CL-42 B-2026-06 = (c) rule gates on OperationDate, weekend purchase legitimately posts (ChargeDate) in-period → fix = ChargeDate∈period. Both = **chunk B4 DONE** (prefix-anchored `InterestCommissionIvaPattern`; CL-42 ChargeDate∈[start,cut] + supplementary OperationDate≤cut; +5 Validation tests → 552). Bonus grounded by evidence: the SAME two bugs were failing the C-series (CL-42 C×4, CL-18 C-2026-03 — post-S4.c verdicts, landed after S3 triage) → CL-18 and CL-42 now **8/8 Pass** on all real credit-card statements, both severity buckets Critical→Info. Bank arithmetic exact again — still ZERO true statement defects. **Chunk B2 DONE** (fiscal block CL-50–53): fast-path now recognizes the real SAT legend family (additive to synthetic); folio-fiscal UUID reassembled across its 2-line wrap; issuer RFC label-anchored past the "RFC Proveedor Certificado" decoy; QR DPI-escalation ladder 150→300 (flat 300 broke a synthetic fixture QR — caught by the cardinal-rule gate); CL-53 rule split NotExtracted→InsufficientData (receiver RFC genuinely never in text layer ×8; malformed still Fails; mirrors CL-21 convention, 1 pinned unit test updated). Real corpus: CL-50/51/52 Fail×8→Pass×7 + C-2026-02 residual (zero legend text — same root as CL-46 residual); CL-53 Fail×8→InsufficientData×8. B-series Fail counts 10→6. **Chunk B3 DONE** (LAW-SEC): ground truth INVERTED the triage expectation — 0/23 section headings exist in the real family's text layer (all graphic-rendered); the old `Contains` matcher's 5 "Present" sections were false positives via 3 non-heading shapes (cross-ref sentences §26/§27, §19-table row label ending in the §16 anchor, definition/legend prose §9/§22). Fix = `IsHeadingLikeMatch` structural heading-shape gate + `IsCrossReferenceSentence` + `ContainsAnotherAnchor` (multi-column compound-heading case §7/§8). Honest outcomes: PRESENCE stays Fail but missing 18→22 (fully honest); ORDER-GAP Fail×8→InsufficientData×8; knock-ons via `DetectedSection`: §16 InsufficientData→Pass×8, §27 Fail×8→InsufficientData×8, **§26 Pass×8→InsufficientData×8 (S4.a's §26 Pass ran against a falsely-located section — honest abstention supersedes; S5 pin = InsufficientData)**. Demo 5/5 no pin movement. Fail counts now B×4=4, C-2026-02=8, C others=4. | DONE | S3 evidence |
| RC1.S5 | **Regression pin.** Pin the post-S4 calibrated expectations as an env-gated `[Theory]` over the corpus index (mirror of the synthetic `corpus-manifest.json` pattern; loud loader + completeness fact — no vacuous zero-row pass). | DONE (`RealCorpusCalibrationPinTests`: 16 exact per-statement pins — Signal/tiers + 16 per-check verdicts each — + completeness fact; MemberData THROWS on present-but-empty index or unpinned entry; +1 CL-42 blind-spot pin in Validation. Orch corpus-present **260**, corpus-absent 225+5skip, Validation **553**, demo 5/5, calibration docs drift-free. Discrepancy honestly pinned: LAW-TYPO-MINSIZE = Fail×8 (B AND C), not the handoff's ×4. Gotcha: corpus-absent CANNOT be simulated on this box by unsetting the env var — locator falls back to the default staging path; point it at a nonexistent path instead) | S4 done |

## Epic close (2026-07-22, second orchestrated session)

S4.b landed in 4 chunks (B1 `4d550573` CL-31/CL-48 · B4 `71e8b8fc` CL-18/CL-42 · B2 `4c55c93f`
CL-50–53 · B3 `a44eff25` LAW-SEC), adversarial gate PASSED, S5 pins landed (this commit).
Final floors: **Extraction 503 / Validation 553 / Orchestration 260 corpus-present (225+5skip
absent) / Application 158 / ReferenceData 46**; demo 5/5 throughout. Real-corpus Fail counts:
B×4=4, C-2026-02=8, other C=4 (was 10–13 at S2 baseline). Still ZERO proven true statement
defects except LAW-TYPO footnote markers (owner-ratified, pinned Fail×8).

**Open residuals (all pinned as-is by S5; reopen only on evidence/owner ruling):**
CL-48 ~211pt truly-empty region ×8 (needs visual/owner review — possible vector graphics
invisible to PdfPig); CL-46 + CL-50/51/52 on C-2026-02 (statement prints no fiscal legend in
text layer); CL-32 + LAW-§27 visual/OCR confirmation (image-rendered, unresolved since S3);
LAW-SEC-PRESENCE honest Fail×8 (all 23 headings graphic-rendered on the real family);
LAW-TYPO ×8 owner-ratified. **Adversarial-gate documented risks (fail LOUD on out-of-corpus
variants):** CL-18 5-prefix fee allowlist; CL-50/51/52 2-variant legend allowlist; CL-42
null-ChargeDate + prior-period blind spot. **Owner-TODO carried from S4.c:** PLATINUM
annualCommission/rewards values ungrounded.

## Prior handoff — superseded (2026-07-22, first orchestrated session end)
