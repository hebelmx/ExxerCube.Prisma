...
| RC1.S4 | **Evidence-gated fixes.** Owner authorized all 3 tracks 2026-07-22 + ruled: CL-46 → SAT "…DE UN CFDI" legend; PLATINUM catalog row in BOTH catalogs; **§26-i → tolerate as accepted variant**; **TYPO footnote markers → stay IN scope (fail stands — first owner-ratified true finding on real statements)**. Sub-stories: S4.a rules (CL-18, §26 matcher+variant, CL-21 implied-zero retirement) · S4.b extractor feeders (CL-31, CL-48, CL-50–53, LAW-SEC anchors+cross-ref guard) · S4.c data (CL-46 legend, catalog rows — both bundles per dual-catalog risk) · S4.d corpus pipeline (anonymize.py auto-shrink → re-anonymize → re-index → re-measure). | IN PROGRESS | S3 evidence |
