...
|---|-----|---------|-------------|
| F1 | Major | Implied-zero cannot distinguish *genuinely-absent* row from *label-present-but-amount-unparseable* — the extractor returns `NotExtracted` (via `ExtractedField.Missing`, `PdfPigStatementFieldExtractor.cs:1441`) for **both**, so a future PDF with a printed-but-unreadable Adeudo/Pagos amount would silently become `0m`. **NOT a problem for `bad-math-cl21`** (rows truly absent) or `good.pdf`. | **DEFERRED — tracked follow-up.** Fix touches the extractor's status model (e.g. return a distinct `ExtractedInvalidFormat` when a label is matched but the amount fails to parse, and have CL-21 guard on it). Broader than Epic 5; candidate for Epic 6 hardening. **Owner decision needed: fix now vs Epic 6.** |
