...
|---|------|--------|----------|----------|
| O1 | ~~**Athena container cannot run OCR**~~ — ✅ **DONE 2026-07-27** (`TRACKER-O1-athena-container-ocr.md`): Emgu runtime package made property-driven (`EmguLinuxRuntimePackage`; container uses `ubuntu-24.04-x64` for the noble base, dev box unchanged), Dockerfile runtime stage got tesseract+spa/eng tessdata+both link shims+libcvextern's noble deps, new `--ocr-smoke` self-test proven in-container (EXIT 0, spa model) and wired as a CI step. **Follow-up ✅ DONE 2026-07-28** (`TRACKER-webui-container-ocr.md`): the Web.UI image had the same Emgu hole (only its Tesseract half was fixed by GH#28) — O1's pattern applied verbatim (package override + noble apt deps + emgu shim + Web.UI-local `--ocr-smoke`, proven EXIT 0 in-image, CI parity step). | RC6 0.9 residual | ~~Blocks~~ CLOSED | `docker run … --ocr-smoke` EXIT 0; CI step in quality-gates.yml |
