...
|---|------|--------|----------|----------|
| O1 | **Athena container cannot run OCR** — `Prisma.Athena.Worker/Dockerfile` is bare `aspnet:10.0`: no tesseract/leptonica, no Emgu/OpenCV natives, no native-link shim, no tessdata. Athena *is* the production OCR host (`TesseractOcrExecutor` @ `Program.cs:161` + interop guard @ :32-34). Only the Web.UI image got the OCR stack (GH#28). Compose `athena` service installs nothing either. | RC6 0.9 residual; flagged 2026-06-26, still valid | **Blocks** (containerized deploy) | grep Dockerfile: zero matches for tesseract/leptonica/emgu |
