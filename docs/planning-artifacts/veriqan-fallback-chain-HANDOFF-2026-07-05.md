...
**What S6.2.6 delivers (`48d19266`):**
- **Batch runner** (`scripts/veriqan-corpus/synth_gen.py`): a hardcoded `CORPUS_SPECIMENS` 12-row source-of-truth table drives `build_corpus_index()`/`_write_corpus_index()`; a new `--write-index` CLI flag writes ONLY `Prisma/Fixtures/PRP2/synthetic/corpus-manifest.json` in index-only mode (refresh the index WITHOUT regenerating/byte-churning the 12 committed PDFs). `--profile all` also (re)writes the index after a full regen. The index is sourced from the table, **never by scanning `OUTPUT_DIR`** (a scan is OS/order-dependent and could index stale files), sorted by `id`, byte-deterministic (regen = byte-identical, proven twice).
- **CI gate** (`Veriqan.Infrastructure.Extraction.Tests`, the same lane the root `run-coverage*.ps1` runners execute): `SyntheticCorpusIndexLoader` (throws LOUDLY on missing/malformed index — no silent zero-data theory); the golden round-trip is now ONE `[Theory]` over `corpus-manifest.json` (same `AssertGoldenRoundTripAsync` assertions, so adding a fixture = adding an index row, not a new test method); a new completeness `[Fact]` (`CorpusIndex_AgreesWithFixturesDirectory_NoOrphansEitherDirection`) asserts index ↔ `Fixtures/synthetic` agree in BOTH directions (orphan fixture with no index row; index row with no file). Gate is on WORD-GEOMETRY (round-trip through PdfPig), NOT PDF bytes.

**Owner rulings folded in (AskUserQuestion, session 4):** (1) scope = S6.2.6 then stop + hand off (epic boundary); (2) CI gate = the dependency-light C# corpus-completeness theory (Python generator stays batch runner only, NO Python in the C# test lane) — the alternative "regen-in-test-host" was rejected (fragile cross-language dep).

**Adversarial review (1 skeptic, 7 refutation angles): SOUND TO COMMIT, 0 defects.** Independently re-verified: coverage preserved 1:1 (the 8 old facts + 3 variance cases → 12 index-driven theory cases, identical assertions); completeness gate catches both drift directions and **backstops the highest-value hole** (an empty/`specimens:[]` index would make all 24 on-disk files orphans → loud fail, so coverage can't silently zero out — and a missing/malformed index throws at discovery); geometry-not-bytes honored everywhere; index byte-deterministic; the SIBLING `Prisma/Fixtures/PRP2/corpus-manifest.json` (the checklist-calibration bundle — a DIFFERENT file) untouched; no PDF churn.

**⚠️ Two by-design boundary notes (carry forward, NOT defects):**
1. The completeness gate proves **index ↔ output-directory** agreement (the copied `Fixtures/synthetic` in test output). A source fixture added to `Fixtures/PRP2/synthetic` but wired into NEITHER `CORPUS_SPECIMENS` NOR the csproj `<Content>` copy escapes the gate. Any NORMAL add path is caught (`--write-index` adds the row → a missing csproj copy then trips "indexed pdf not found on disk"). The standing corpus is *defined by* the table — this is intentional scope.
2. The two S6.2.4 edge rows carry index `defect: "edge-band"/"edge-label"` while their manifests say the shared `"edge"` — deliberate (more informative index slugs); `defect` is documentation only, not modeled in `SyntheticCorpusSpecimen`, asserted nowhere.

**Forward item still open from S6.2.5 (unchanged):** the `movements[]` array is honest/reproducible but NOT asserted — `AssertGoldenRoundTripAsync` reads only the `fields` map and `StatementModelFieldAccessors.Map` has no `movements` accessor. A movements-level assertion is a model-widening slice; deferred (totals-level proof suffices for now). **LESSON: a data-driven `[Theory]` with zero rows passes as "no failures" — always pair it with a loud-throwing loader + a completeness gate so broken/empty data fails instead of silently zeroing coverage.**

### 🎯 NEXT SESSION — epic E6.S6.2 is DONE; pick the next epic with the owner
Per the design program plan, remaining PI-1 fallback-chain epics: **E3** (validators grow alongside — ladder-exhaustion honesty, task #5), **E5** (LLM stage — `FieldCandidate` LLM model/hash provenance, task #7), **E4** (semantic, only after E2 proves residual gaps), **E7** (real product resolution — retires the alias hack, gate re-spec). The synthetic corpus now exists as the measurement harness for all of them. Do NOT roll into one without an owner ruling on scope. Verify EVERY chunk against the full verdict pipeline (§4); calibrate from PdfPig coords.

---

## ⏩⏩⏩⏩⏩⏩⏩ (2026-07-07 session 3) — **E6.S6.2.5 (populated DESGLOSE movements + totals) DONE + reviewed + pushed `03197966` → NEXT = S6.2.6 (batch + CI)**
