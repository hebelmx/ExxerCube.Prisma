...
### S4-M.1.0 — Diversification spike VERIFIED RESULT (2026-07-19) — owner ruling: diversify before growing

Growing by over-sampling the one observed bug (`FI1` I→l) would be a **monoculture** → the S4-M.2 precision number would measure a single glyph bug 30×, not real fallback precision. Owner ruled **diversify first**. Spike empirically confirmed **3 DISTINCT, grounded, source-contained** deterministic-NULL modes across **3 pipeline layers** (each reproduced via CLI tesseract w/ prod-matched config `-l spa --oem 1 --psm 6` + prod char-whitelist, `pdftoppm -r 300`, and a byte-for-byte replay of `AdaptiveTxtFieldExtractor.ExtractExpediente`):

| # | Mode | Layer | Mechanism | Generator knob | Evidence |
|---|------|-------|-----------|----------------|----------|
| 1 | `FI1`→`Fl1` glyph | OCR glyph | Tesseract reads capital `I` as lowercase `l` in the `FI1` token → `[A-Z]{1,4}` breaks | `area_codes=['FI1']` **AND exclude `judicial` type** (see rescue caveat) | 5/5 null, source-contained; FI1-specific (other 6 codes + suffix-`I` all clean) |
| 2 | em-dash `—` dropped | OCR char-whitelist | prod `TesseractOcrExecutor` whitelist has no `—`; em-dash is DROPPED (not substituted) → `A/AS11234567890AAA`, no delimiter → null | delimiter-override knob (em-dash) | 2/2 null, source-contained |
| 3 | spaced hyphen `" - "` | pure regex-strictness | Tesseract reads it PERFECTLY; `[-–]` has zero whitespace tolerance → `\d+[-–]\d+` can't span → null despite flawless OCR | delimiter-override knob (spaced/period) | 3/3 null, source-contained (period `.` + `--` corroborate same root cause) |

**REJECTED (tested, did not reproduce a grounded source-contained null):** 6/7 non-FI1 area codes; suffix-position `I` (`UIF`,`IMX`); en-dash `–` (OCR normalizes back to `-`, and regex already accepts `–`); digit-`1`-as-letter misreads; **narrow-container line-wrap** (fails literal source-containment — `pdftotext` hyphenation-collapse mangles it — AND requires undoing the generator's shipped "EXPEDIENTE CONTIGUITY" fix; that fix is load-bearing).

**Structural caveat (mode 1):** `judicial`-type docs (75% of the golden recipe) ALSO embed the Expediente inline in Motivación prose (`legal_catalog.py:183`), which RESCUES a dedicated-line misread → mode 1 must emit **non-judicial** docs.
