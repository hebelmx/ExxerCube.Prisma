...
| C2.C | Cl42/Cl49 date-window guard-gap — evaluate; log honestly if inert (do not build for scope's sake) | ❌ **NO-GO (vacuous-by-design, 2026-07-18)** | Read-only scout + 3 ground-truth spot-checks. The money guard fires on `ExtractedField<T>.Confidence < 0.8`, a sub-1.0 value produced **only** by the geometric scorer for MONEY/RATE fields (`FieldCalibrationTable` keys = TasaCat/Resumen/TotalRow/HeaderMoney only — `GeometricPlausibilityScorer.cs:265-349`). **No date equivalent exists:** `PeriodStart`/`PeriodCutDate` are `ExtractedField<DateOnly>` but every extraction path emits them via the constant-1.0 `Found(value, locator)` overload (`PdfPigStatementFieldExtractor.cs` `ExtractPeriodStart`:959, `ExtractFechaDeCorte`:1050, `ExtractFechaLimiteDePago`:1103 — no scorer, no `_emitGeometricConfidence`) → `Confidence` always 1.0; and `StatementMovement.OperationDate` is a bare `DateOnly?` (`StatementMovement.cs:88`) with **no confidence carrier at all**. So a guard gates on a constant 1.0 (or nothing), can never trip, and cannot change a verdict on real `good.pdf` — CL-42 currently **Passes non-vacuously** (`MovementRulesTests.cs` `Cl42_AllDatesInPeriod_ReturnsPass`) and is not in good.pdf's 13 `FailCheckIds`; CL-49 is **structurally incapable of a hard FAIL** by design (expired-promo → InsufficientData, `Cl49…Rule.cs:59-60,151-160`) and its only extracted date input is the constant-1.0 `PeriodStart` (`promo.ValidTo` is a reference-bundle string, not PDF-extracted). Sole residual — a misread that parses to a well-formed-but-wrong date — carries no competing-token / positional / calibration signal and is indistinguishable from a correct date; nothing to detect. Same terminus as C2.3/C2.4. **Armable only if someone first BUILDS + emits a date-plausibility confidence (new extractor work + a `Confidence` carrier on `OperationDate`) — a guard retrofit alone is inert. NOT built (owner-skeptical; "do not build for scope's sake").** Slice C (weak; owner authorized but skeptical). |
...
- 2026-07-18 — **C2.C NO-GO (vacuous-by-design). EPIC C2 CLOSED.** Evaluation-first per owner-skeptical
  authorization ("evaluate; do not build inert guards for scope's sake"). One read-only scout traced the
  date path from ground truth; orchestrator spot-checked the 3 load-bearing facts directly. The 0.8 guard
  consumes `ExtractedField<T>.Confidence`, a sub-1.0 value emitted **only** by the geometric scorer for
  money/rate fields — `FieldCalibrationTable` has no date key, the three date extractors use the constant-1.0
  `Found(value, locator)` overload, and `OperationDate` has no `Confidence` carrier. A date guard therefore
  gates on a constant 1.0 (or nothing), can never trip, and cannot flip good.pdf (CL-42 Passes non-vacuously;
  CL-49 cannot hard-FAIL by design). The only residual — a well-formed-but-wrong misread — is signalless and
  undetectable. Making it armable would require BUILDING a new date-plausibility confidence (new extractor
  work), not a guard retrofit → out of scope + owner-skeptical. **Zero code written; docs-only close.** The
  armable confidence surface for the whole C2 epic is now fully exhausted (C2.1b armed; C2.3/C2.4/C2.C all
  NO-GO vacuous; C2.5 drift-guard holds). **No open tasks remain — C2 tracker complete.**
