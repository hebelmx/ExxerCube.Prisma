...
| **C2.1b** | Promote refined mechanism → production `HeaderMoneyFieldGeometricSignals`/`ScoreHeaderMoneyField` + `FieldCalibrationTable.HeaderMoney` (AC3: literal prod `GetBand`); wire `ExtractNivelDeUsoField` + `ExtractPagoParaNoGenerarIntereses`; emit goes **LIVE** (armed flag) | TODO | Only after C2.1a GO. AC2 (tighten decoy expectedStatus) applies on arm. **DO NOT wire the prototype verbatim.** |
