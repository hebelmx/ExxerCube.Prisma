...
**Phase 6 addenda (the closing phase — fix-impl-then-test, the inverse of Phases 1–5; full lessons in
`docs/development/lessons-learned/2026-06-11-itdd-phase-6-closing.md`):**

- **`ResultExtensions.Cancelled` and the `IsCancelled()` extension both live in `IndQuestResults.Operations`,
  not `IndQuestResults`.** Files that only `using IndQuestResults;` compile until they reference
  `ResultExtensions` → `CS0103`. Add the `.Operations` using to each impl AND each mock factory AND each
  contract base that adds a cancel test. (Classification's `GlobalUsings.cs` already had it.)
- **Cancellation mutation-cleanliness is automatic IFF every guarded Result method has both a success test
  and a cancel test.** The guard's three mutants: condition→`true` (killed by any existing success test, which
  now gets Cancelled), condition→`false` (CS0162 unreachable → Stryker CompileError, discarded — not a
  survivor) and block-removal (killed by the new cancel test). So "guard every method → cancel-test every
  Result method" is exactly Survived-0. Empirically: Export.Adaptive (15 guards) + Extraction (1 guard) scoped
  Stryker reported **0 cancellation-guard survivors** (parse the report by survived-mutant line text — the
  headline % is noisy and the population shifts when you scope `--mutate`).
- **For a disproportionately huge mutation-hardened file (Fusion, 2789 lines), use the §4-sanctioned manual
  block-removal spot-check instead of a full re-mutation.** Neuter the guard at *runtime* (replace the
  condition with `cancellationToken.IsCancellationRequested && !cancellationToken.IsCancellationRequested`
  — NOT `false`, which is CS0162-as-error), build, run the cancel tests, confirm EXACTLY they fail, revert.
  Definitive and ~3 min vs ~40+ min.
- **`StrykerCompat=true` flattens shared `obj/ref`, so a background Stryker run BLOCKS concurrent
  `dotnet build`** (`CS0006: …/ref/….dll could not be found`). Sequence Stryker and builds; don't background
  Stryker if you need the build for other tasks. A fresh normal `dotnet build` clears the flattened state.
- **A fixed impl invalidates the test that pinned the bug** — grep for tests asserting the OLD behaviour
  before committing an impl fix (here the EfCoreRepository not-found fix broke one integration test that had
  asserted the buggy `IsSuccessMayBeNull`+null; updated to `IsFailure`).
- **Sweep discipline ("no silent caps"):** `FieldSanitizer`/`FieldPatternValidator` are **static classes**
  (no interface, single impl) → the ADR-005 inject-a-Sut/N+1 pattern does not apply; their `…ContractTests`
  name is descriptive, not the ADR-005 shape — SKIP, don't convert. The **`IFieldExtractor<T>` family**
  (Xml/Docx/Pdf/Txt adapters over distinct source types) is the one real deferred candidate — own-phase
  effort, each adapter already individually+mutation-tested, and the interface lacks a CancellationToken.
...
| 6 | Sweep + guardrails + docs + carried impl-fixes | ✅ Done (2026-06-11) — **the closing phase; it LANDS the carried fixes (production behaviour changes, owner-gated), then sweeps/guardrails/docs.** **(A1) EfCoreRepository not-found dead-guard fixed** — `IsSuccess && Value is null` → `IsSuccessMayBeNull && Value is null` (both GetByIdAsync/FirstOrDefaultAsync); the 2 `RepositoryContract` not-found tests tightened `IsSuccess.ShouldBeFalse()`→`IsFailure.ShouldBeTrue()`; the one integration test asserting the buggy behaviour updated→`IsFailure` (renamed). Caller check: only generic consumer `FileMetadataQueryService` already routed not-found to failure. Verified: EfCoreRepositoryContractTests 21/21 vs real EF-InMemory repo (incl. tightened not-found), blueprints 227/227. **(A2) CancellationToken honored across the 6 carried impls + FileTypeIdentifierService** (owner chose full-mandate scope): guard on EVERY public async method + a contract cancel test per Result-returning method (24 new: TemplateFieldMapper 5, TemplateRepository 3, SchemaEvolutionDetector 4, AdaptiveExporter 6, ExpedienteClasifier 4, Fusion 2); each reference fake updated to honor cancel. TemplateRepository's 3 non-Result getters guarded pragmatically (null/empty, no contract test — can't express Cancelled). **(A2b/C-FTI)** FileTypeIdentifierService converted to a contract instance (`FileTypeIdentifierServiceContractTests`), redundant Pdf/UnknownExtension impl tests removed (covered by base), XML/DOCX + message pins kept impl-side. **(B3) Architecture guardrail** added (`Contract_Bases_Must_Be_Abstract_With_At_Least_One_Inheritor`): Tests.Architecture **19→20**. **(B4) Sweep:** no orphaned mock-SUT blueprints remain; `FieldSanitizer`/`FieldPatternValidator` consciously SKIPPED (static utility classes, no interface → ADR-005 N+1 pattern N/A); **`IFieldExtractor<T>` family consciously DEFERRED** (own-phase effort — 4–5 adapters over distinct source types, each already individually+mutation-tested, and the interface lacks CancellationToken; logged in lessons). **(B5) Docs:** superseded iitdd-contract-test-tasks.md; rewrote guidelines §6.5 to the N+1 shape (both copies); explicit mutation-exclusion list (EF/DB + ManualReviewerService); primer already aligned. **Counts exact: Tests.Domain.Interfaces 227→251 (+24 cancel blueprint runs); Export.Adaptive 173→191 (+18); Classification ExpedienteClasifier 23→27 + Fusion 9→11 (+6); Extraction FileTypeIdentifier instance +6 base −2 trimmed.** **(B6) Full-solution: build 0/0, Tests.Architecture 20/20, Tests.Application 572/572 (incl. A1 caller), Tests.Domain 187/187, blueprints 251/251, Export.Adaptive 191/191.** Docker/Playwright-gated suites (Testcontainers SQL incl. the not-found integration test, Ollama, Tests.System/UI/E2E) deferred to a capable box (Docker down here) — consistent with prior phases; A1's not-found integration change verified by equivalence (EF-InMemory contract proves the same real EfCoreRepository class). **MUTATION (cancellation guards mutation-clean — pass signal Survived-0 / NoCoverage==floor):** Export.Adaptive scoped Stryker Killed 424 / Survived 47 / Timeout 0 — **0 of the 47 are cancellation-guard mutants** (parsed from report; 47 = the unchanged Serilog/ConfigureAwait floor, Phase-2's 64 minus AdaptiveResponseExporterAdapter's). Extraction (FileTypeIdentifier) Killed 60 / Survived 8 / Timeout 0 — **0 cancellation-guard survivors**. Classification (ExpedienteClasifier 4 guards + Fusion 2): the 2789-line Fusion full re-mutation was disproportionate, so a **§4-sanctioned manual block-removal spot-check** — neutering all 6 guards (runtime-false) made EXACTLY the 6 cancel tests fail (4+2) and nothing else → all 6 killable; reverted, re-verified green. TemplateRepository Stryker-excluded (EF/DB). **Lessons:** `docs/development/lessons-learned/2026-06-11-itdd-phase-6-closing.md`. **GATE GO** (`/itdd-adversarial-review phase-6`, 2026-06-11, 3 reviewer subagents + cross-check): R1 (tautology+classification) and R3 (mechanics+arch+process) clean GO — 24 cancel tests are non-tautologies (a bare `Substitute.For<T>()` returns default Result → IsCancelled false → test fails; the fakes explicitly honor the token), the 2 not-found tests TIGHTENED not weakened, no existing base assertion loosened, Testing.Contracts csproj/packages untouched, the new arch rule sound (abstract+inheritor, generic-base FullName match, non-vacuous guard), tracker/commits/docs all git-verified. **R2 (coverage+mutation) returned NO-GO with 2 Blockers + 2 Majors claiming 18 Export.Adaptive + 1 Extraction cancel tests FAIL (`IsCancelled()`==false, "ResultExtensions.Cancelled broken") — ALL DISCARDED on reproduction:** R2's `dotnet test` incremental-built against STALE impl assemblies in the StrykerCompat-flattened obj/ref state (the killed background Stryker run), so it ran the cancel tests against the pre-cancel-guard impls. On a **forced-clean rebuild** (`--no-incremental`): **18/18** Export.Adaptive cancel tests + **6/6** FileTypeIdentifier contract tests PASS; the manual spot-check independently proved the cancel tests fail ONLY when the guard is neutered → `IsCancelled()` works. This is the §4.1-step-11 StrykerCompat-flattened-artifact flap that bit the Phase 1/2 gates (a reviewer rebuilding atop StrykerOutput reads false failures). **0 reproduced Blocker/Major.** **ITDD REFACTOR (Phases 0–6) COMPLETE.** | Kt2 `a3d50a8` (A1) + `b6936fa` (A2/A2b) + `3451203` (B5) + B-close commit |
