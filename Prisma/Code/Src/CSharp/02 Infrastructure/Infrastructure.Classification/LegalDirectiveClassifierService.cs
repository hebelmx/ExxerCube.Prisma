...
public class LegalDirectiveClassifierService : ILegalDirectiveClassifier
{
    ...
    /// <summary>
    /// Delegates to <see cref="RequirementDetailExtractor"/> — single source of truth for regex patterns.
    /// </summary>
    private static void ExtractActionDetails(string text, ComplianceAction action)
    {
        action.AccountNumber = RequirementDetailExtractor.ExtractFirstAccountNumber(text);

        var (amount, _) = RequirementDetailExtractor.ExtractAmountAndCurrency(text);
        action.Amount = amount;

        var products = RequirementDetailExtractor.ExtractProducts(text);
        action.ProductType = products.Count > 0 ? products[0] : null;
    }
