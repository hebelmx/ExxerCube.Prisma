...
var app = Prisma.Reconciliator.Worker.Program.BuildApp(args);

if (args.Contains("--migrate-only"))
{
    using var cts = new CancellationTokenSource();
    Console.CancelKeyPress += (_, e) => { e.Cancel = true; cts.Cancel(); };
    var exitCode = await ExxerCube.Prisma.Infrastructure.Database.Startup.PrismaDbMigrationRunner.RunMigrationsAsync(app, cts.Token);
...
namespace Prisma.Reconciliator.Worker
{
    ...
    {
        /// <summary>
        /// Builds the fully-wired worker <see cref="WebApplication"/> (identical to the production
        /// composition root). <paramref name="configureEarly"/> runs immediately after
        /// CreateBuilder (so a test can inject configuration BEFORE the inline config reads), and
        /// <paramref name="configureServicesLate"/> runs immediately before <c>builder.Build()</c>
        /// (so a test can remove/replace registrations). Both are null in production (no-ops).
        /// </summary>
        public static WebApplication BuildApp(
            string[] args,
            Action<WebApplicationBuilder>? configureEarly = null,
            Action<IServiceCollection>? configureServicesLate = null)
        {
            var builder = WebApplication.CreateBuilder(args);
            configureEarly?.Invoke(builder);
            ...
            configureServicesLate?.Invoke(builder.Services);
